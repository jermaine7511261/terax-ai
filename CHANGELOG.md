...
### 新增
- **第十九轮（0.1.20）** 模型/智能体状态栏增强：① 模型与智能体分离——`AgentSwitcher` 拆出独立 `ModelSwitcher`，状态栏两个并列下拉（`AiStatusBarControls`）；② 模型选择持久化——`selectedModelId` 存入 preferences，启动经 `useRestoreSelectedModel` 恢复；③ 状态栏上下文占用百分比——新增 `contextEstimate`（`approxBytesFromUI`，与压缩逻辑 `compactModelMessagesDetailed` **同源的近似 token**）÷ `getModelContextLimit`，`ModelSwitcher` 触发器显示 `NN%`，≥55% 琥珀 / ≥70% 红（对应压缩触发阈值），参考 Hermes 状态栏 context_pct；④ 思考长度选项卡——`ModelSwitcher` 下拉新增思考长度档位（off/low/medium/high，持久化 `thinkingLength`），经 `providerOptions.reasoning_effort` 传入模型请求。tsc + vitest 1646 + i18n-scan + drift + lint 全绿。
