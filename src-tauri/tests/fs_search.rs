...
fn grep_finds_matches_and_returns_relative_paths() {
    ...
    let res = fs_grep_impl("hello".into(), fx.root_str(), None, None, None, None, None, None, None)
    .expect("grep");
...
fn grep_case_insensitive_finds_mixed_case() {
    ...
    let strict = fs_grep_impl("hello".into(), fx.root_str(), None, Some(false), None, None, None, None, None)
        .expect("grep");
    assert!(strict.hits.is_empty());
    let loose = fs_grep_impl("hello".into(), fx.root_str(), None, Some(true), None, None, None, None, None)
        .expect("grep");
...
fn grep_glob_filter_restricts_files() {
    ...
    let res = fs_grep_impl("target".into(), fx.root_str(), Some(vec!["*.rs".into()]), None, None, None, None, None, None)
    .expect("grep");
    assert_eq!(res.hits.len(), 1);
...
fn grep_max_results_truncates() {
    ...
    let res = fs_grep_impl("needle".into(), fx.root_str(), None, None, Some(3), None, None, None, None)
        .expect("grep");
...
fn grep_empty_pattern_errors() {
    let fx = FsFixture::new();
    let err = fs_grep_impl("".into(), fx.root_str(), None, None, None, None, None, None, None);
    assert!(err.is_err());
}
#[test]
fn grep_non_dir_root_errors() {
    let err = fs_grep_impl("x".into(), "/this/does/not/exist".into(), None, None, None, None, None, None, None);
    assert!(err.is_err());
}
#[test]
fn grep_respects_ignore_file() {
    ...
    let res = fs_grep_impl("secret".into(), fx.root_str(), None, None, None, None, None, None, None)
        .expect("grep");
...
fn glob_finds_files_by_pattern() {
    ...
    fx.write("src/b.rs", "");
    fx.write("README.md", "");
    let res = fs_glob_impl("**/*.rs".into(), fx.root_str(), None, None, None, None).expect("glob");
...
fn glob_truncates_on_limit() {
    ...
    let res = fs_glob_impl("*.txt".into(), fx.root_str(), Some(5), None, None, None).expect("glob");
...
fn glob_empty_pattern_errors() {
    let fx = FsFixture::new();
    assert!(fs_glob_impl("".into(), fx.root_str(), None, None, None, None).is_err());
}
#[test]
...
fn read_dir_orders_dirs_before_files_then_alpha() {
    ...
    fx.write("zfile.txt", "");
    fx.write("afile.txt", "");
    let entries = fs_read_dir_impl(fx.root_str(), false, None, None, None, None).expect("read_dir");
...
fn read_dir_hides_dotfiles_by_default() {
    ...
    let hidden_off = fs_read_dir_impl(fx.root_str(), false, None, None, None, None).expect("read_dir");
    let names: Vec<&str> = hidden_off.iter().map(|e| e.name.as_str()).collect();
    assert_eq!(names, vec!["visible.txt"]);
    let hidden_on = fs_read_dir_impl(fx.root_str(), true, None, None, None, None).expect("read_dir");
...
fn read_dir_flags_gitignored_entries_only_when_requested() {
    ...
    let entries = fs_read_dir_impl(fx.repo_str(), false, Some(true), None, None, None).expect("read_dir");
    let flag = |name: &str| {
        entries
            .iter()
            .find(|e| e.name == name)
    ...
    assert!(flag("build"));
    let plain = fs_read_dir_impl(fx.repo_str(), false, None, None, None, None).expect("read_dir");
...
fn read_dir_skips_gitignore_outside_a_repo() {
    ...
    fx.write("kept.txt", "");
    let entries = fs_read_dir_impl(fx.root_str(), false, Some(true), None, None, None).expect("read_dir");
