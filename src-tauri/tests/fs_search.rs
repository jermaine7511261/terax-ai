...
fn grep_case_insensitive_finds_mixed_case() {
    ...
    let strict = fs_grep_impl("hello".into(), fx.root_str(), None, Some(false), None, None, None, None)
        .expect("grep");
    assert!(strict.hits.is_empty());
    let loose = fs_grep_impl("hello".into(), fx.root_str(), None, Some(true), None, None, None, None)
        .expect("grep");
...
fn grep_max_results_truncates() {
    ...
    let res = fs_grep_impl("needle".into(), fx.root_str(), None, None, Some(3), None, None, None)
        .expect("grep");
...
fn grep_empty_pattern_errors() {
    let fx = FsFixture::new();
    let err = fs_grep_impl("".into(), fx.root_str(), None, None, None, None, None, None);
    assert!(err.is_err());
}
#[test]
...
fn grep_respects_ignore_file() {
    ...
    let res = fs_grep_impl("secret".into(), fx.root_str(), None, None, None, None, None, None)
        .expect("grep");
