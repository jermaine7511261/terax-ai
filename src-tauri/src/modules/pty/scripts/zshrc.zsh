...
# zsh, so we shadow $? into `_yamet_ret`.

{
  _yamet_user_zdotdir="${YAMET_USER_ZDOTDIR:-$HOME}"
  [ -f "$_yamet_user_zdotdir/.zshrc" ] && source "$_yamet_user_zdotdir/.zshrc"
...
if [[ -z "$__YAMET_HOOKS_LOADED" ]]; then
  __YAMET_HOOKS_LOADED=1
  ...
  _yamet_precmd() {
    local _yamet_ret=$?
    printf '\e]133;D;%s\e\\' "$_yamet_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_yamet_urlencode "$PWD")"
  ...
  _yamet_preexec() {
    ...
    [[ -n "$YAMET_BLOCKS" ]] && _yamet_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _yamet_precmd
    add-zsh-hook preexec _yamet_preexec
  fi
