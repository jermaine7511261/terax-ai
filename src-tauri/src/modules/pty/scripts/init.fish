...
set -g __YAMET_HOST (uname -n 2>/dev/null; or echo localhost)

# URL-encode a path keeping `/` intact so it stays valid inside file://.
function __yamet_urlencode_path
...
function __yamet_restore_status
    return $argv[1]
end

function __yamet_capture_user_prompt
    if not functions -q fish_prompt
        return
    end
    if functions fish_prompt | string match -q '*__yamet_user_prompt*'
        return
    end
    functions -e __yamet_user_prompt 2>/dev/null
    functions -c fish_prompt __yamet_user_prompt
end

# Wrapped so `fish -C __yamet_install_prompt` can re-run it AFTER config.fish,
# where a framework prompt (starship etc.) would otherwise override fish_prompt
# and drop our markers.
function __yamet_install_prompt
    __yamet_capture_user_prompt
    if set -q YAMET_BLOCKS
    ...
    function fish_prompt
        set -l __yamet_status $status
        printf '\e]133;D;%d\e\\' $__yamet_status
        printf '\e]7;file://%s%s\e\\' "$__YAMET_HOST" (__yamet_urlencode_path "$PWD")
        printf '\e]133;A\e\\'
        ...
        __yamet_restore_status $__yamet_status
        if functions -q __yamet_user_prompt
            __yamet_user_prompt
        else
            printf '%s > ' (prompt_pwd)
        end
...
__yamet_install_prompt

function __yamet_preexec --on-event fish_preexec
    set -g __yamet_block_seen 1
