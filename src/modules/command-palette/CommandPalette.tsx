...
export function CommandPalette({
  ...
  const placeholder = inThemes
    ? translate("commandPalette.placeholderThemes")
    : parsed.mode === "content"
      ? translate("commandPalette.placeholderContent")
      : parsed.mode === "history"
        ? translate("commandPalette.placeholderHistory")
        : translate("commandPalette.placeholder");

  return (
    <CommandDialog
      open={open}
      onOpenChange={handleOpenChange}
      title={translate("commandPalette.title")}
      description={translate("commandPalette.description")}
      ...
      >
        ...
        <ScrollArea className="max-h-[420px]">
          <CommandList className="max-h-none overflow-visible pr-3">
            {inThemes ? (
              <CommandGroup heading={translate("commandPalette.themesHeading")}>
                <CommandItem
                  value="theme:back"
                  onSelect={exitThemes}
                ...
                >
                  <HugeiconsIcon
                    ...
                    strokeWidth={1.75}
                  />
                  <span>{translate("commandPalette.back")}</span>
                </CommandItem>
                {themes.map((t) => (
                  <CommandItem
                    key={t.id}
                ...
                {themes.length === 0 ? <StatusItem label={translate("commandPalette.noThemes")} /> : null}
              </CommandGroup>
              ...
              <CommandGroup heading={translate("commandPalette.contentsHeading")}>
                {!workspaceRoot ? (
                  <StatusItem label={translate("commandPalette.noWorkspaceRoot")} />
                ) : parsed.term.length < CONTENT_SEARCH_MIN_QUERY ? (
                  <StatusItem label={translate("commandPalette.typeMinChars")} />
                ) : (
                  <AsyncBody
                    loading={content.loading}
                    error={content.error}
                    empty={content.results.length === 0}
                    emptyLabel={translate("commandPalette.noMatches")}
              ...
              <CommandGroup heading={translate("commandPalette.historyHeading")}>
                {!insertCommand ? (
                  <StatusItem label={translate("commandPalette.openTerminalForHistory")} />
                ) : (
                  <AsyncBody
                    loading={history.loading}
                    error={history.error}
                    empty={history.results.length === 0}
                    emptyLabel={translate("commandPalette.noHistory")}
              ...
              <CommandGroup heading={translate("commandPalette.searchModesHeading")}>
                {MODE_HINTS.map((hint) => (
                  <CommandItem
                    key={hint.sigil}
...
function AsyncBody({
  ...
  if (error) {
    return (
      <>
        <StatusItem label={translate("commandPalette.searchFailed")} tone="error" />
        <CommandItem value="retry" onSelect={onRetry} className="text-[12.5px]">
          <span>{translate("commandPalette.retry")}</span>
        </CommandItem>
      </>
    );
  }
  if (empty && loading) return <StatusItem label={translate("commandPalette.searching")} />;
...
function EmptyHint() {
  ...
  return (
    <div className="flex flex-col items-center gap-2 px-4 py-10 text-center text-sm text-muted-foreground">
      ...
      <span>{translate("commandPalette.noCommandsFound")}</span>
    </div>
  );
}
