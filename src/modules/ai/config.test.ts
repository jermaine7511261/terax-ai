...
describe("resolveModel", () => {
  ...
  it("resolves a static model id from the registry", () => {
    expect(resolveModel("deepseek-v4-flash").provider).toBe("deepseek");
  });
...
describe("current model pricing", () => {
  it.each([
    ["deepseek-v4-pro", 0.28, 1.1, 0.028],
    ["deepseek-v4-flash", 0.07, 0.27, 0.007],
...
describe("modelKeepsReasoning", () => {
  ...
  it("drops reasoning for plain non-reasoning models", () => {
    expect(modelKeepsReasoning(resolveModel("codestral-latest"))).toBe(false);
  });

  it("keeps reasoning for tagged reasoning models", () => {
    expect(modelKeepsReasoning(resolveModel("deepseek-reasoner"))).toBe(true);
  });
});
...
describe("model sampling capabilities", () => {
  ...
  ] as const)("keeps temperature for %s/%s", (provider, modelId) => {
    expect(modelSupportsTemperature(provider, modelId)).toBe(true);
  });
