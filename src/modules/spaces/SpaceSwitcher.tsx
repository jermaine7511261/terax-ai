  ...
  const pickRoot = async (id: string) => {
    try {
      const dir = await openDialog({
        directory: true,
        multiple: false,
        title: t("spaces.chooseFolder"),
      });
      if (typeof dir === "string" && dir) setRoot(id, dir);
    } catch {
      // cancelled
    }
  };
        ...
        <div className="-mx-0.5 max-h-[60vh] overflow-y-auto px-0.5">
          {spaces.map((sp) => (
            <SpaceRow
              ...
              onPickRoot={() => void pickRoot(sp.id)}
              onJumpTab={onJumpTab}
...
function SpaceRow({
  ...
  return (
    <div className={cn("relative", isDragging && "opacity-50")}>
      ...
      <div data-no-drag className="flex items-center gap-1.5 pl-10 pr-1.5 pt-1">
        <button
          type="button"
          onClick={onPickRoot}
          title={t("spaces.chooseFolder")}
          aria-label={t("spaces.chooseFolder")}
          className="flex min-w-0 items-center gap-1 rounded px-1 py-0.5 text-[10.5px] text-muted-foreground/70 transition-colors hover:bg-accent hover:text-foreground"
        >
          <HugeiconsIcon icon={Folder01Icon} size={11} strokeWidth={1.75} />
          <span className="truncate">
            {space.root || t("spaces.noRoot")}
          </span>
        </button>
      </div>
