...
/** Full-screen AI chat. The default home tab — no terminal involved. */
export type ChatTab = TabBase & {
  id: number;
  kind: "chat";
  title: string;
};
...
export function useTabs(_initial?: Partial<TerminalTab>) {
  ...
  useEffect(() => {
    ...
    (
      window as unknown as { __yametNewBlockTab?: (cwd?: string) => number }
    ).__yametNewBlockTab = newBlockTab;
  }, [newBlockTab]);
  ...
  /**
   * Activates the chat tab in the active space, creating it if needed.
   * Called when a message is sent so the conversation is always visible
   * full-screen instead of a floating popup.
   */
  const openChatTab = useCallback(() => {
    const existing = tabsRef.current.find(
      (t) => t.kind === "chat" && t.spaceId === activeSpaceIdRef.current,
    );
    if (existing) {
    ...
    setTabs((t) => [
      ...t,
      { id, kind: "chat", spaceId: activeSpaceIdRef.current, title: "chat" },
    ]);
    setActiveId(id);
    return id;
  }, []);
