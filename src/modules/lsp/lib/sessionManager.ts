...
async function createSession(
  ...
  const [{ TauriLspTransport }, { YametLspClient }] = await Promise.all([
    import("./transport"),
    import("./client"),
  ]);

  if (YametLspClient.hostPid === null) {
    YametLspClient.hostPid = await invoke<number>("lsp_host_pid").catch(
      () => null,
    );
  }
