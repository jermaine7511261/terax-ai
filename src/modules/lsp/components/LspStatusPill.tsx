...
const PILL_CLASS =
  "yamet-pill-in ml-1.5 flex h-6 shrink-0 cursor-pointer [&_button]:cursor-pointer items-center gap-1 rounded-full border border-border/50 bg-accent/50 px-2 text-[10.5px] font-medium text-muted-foreground transition-colors duration-200 hover:bg-accent hover:text-foreground";
    ...
    return (
      <span key={`enable-${hint.preset.id}`} className={PILL_CLASS}>
        <button
          ...
          title={t("settingsLsp.startForWorkspace", {
            command: hint.preset.command,
          })}
        >
          <HugeiconsIcon icon={SourceCodeIcon} size={11} strokeWidth={2} />
          <span>
            {t("settingsLsp.enableName", { name: hint.preset.name })}
          </span>
        </button>
    ...
    <Popover>
      ...
      >
        <div className="mb-1 font-medium text-foreground">
          {t("settingsLsp.stoppedDetail", { name: preset.name })}
        </div>
    ...
    <Popover>
      <span className={PILL_CLASS}>
        <PopoverTrigger asChild>
          <button type="button" className="flex items-center gap-1">
            <HugeiconsIcon icon={SourceCodeIcon} size={11} strokeWidth={2} />
            <span>
              {t("settingsLsp.installName", { name: preset.name })}
            </span>
          </button>
        </PopoverTrigger>
      ...
      >
        ...
        <p className="mb-2 text-muted-foreground">
          {t("settingsLsp.noBinaryBefore")}
          <code className="text-foreground">{preset.command}</code>
          {t("settingsLsp.noBinaryAfter")}
        </p>
        {install ? (
          <div className="mb-2 flex items-center gap-1 rounded-md border border-border bg-muted/40 px-2 py-1.5 font-mono text-[11px]">
            ...
            <button
              ...
              onClick={copy}
              title={t("settingsLsp.copyCommand")}
            >
              <HugeiconsIcon
        ...
        <div className="flex items-center justify-between">
          ...
          >
            {checking
              ? t("settingsLsp.checkingAgain")
              : t("settingsLsp.checkAgain")}
          </button>
        </div>
      </PopoverContent>
    </Popover>
    ...
    <Popover>
      <PopoverTrigger asChild>
        <button
          ...
          className={PILL_CLASS}
          title={
            starting ? t("settingsLsp.starting") : t("settingsLsp.active")
          }
        >
          {starting ? (
            <HugeiconsIcon
      ...
      >
        ...
        <p className="mb-2 text-muted-foreground">
          {t("settingsLsp.runningFor", {
            command: preset.command,
            state: starting
              ? t("settingsLsp.isStarting")
              : t("settingsLsp.isRunning"),
          })}
        </p>
        <div className="flex items-center gap-1.5">
          <button
