...
export default function App() {
  ...
  // Sending a message activates the full-screen chat tab so the conversation
  // is always visible, instead of popping up a floating window. We skip
  // awaiting-approval so an open AI diff tab keeps focus during review.
  const agentStatus = useChatStore((s) => s.agentMeta.status);
  useEffect(() => {
    if (agentStatus === "thinking" || agentStatus === "streaming") {
      openChatTab();
    }
  }, [agentStatus, openChatTab]);

  const activeTab = tabs.find((t) => t.id === activeId);
  const isTerminalTab = activeTab?.kind === "terminal";
  const isChatTab = activeTab?.kind === "chat";
  ...
  const handleAttachFileToAgent = useCallback(
    (path: string) => {
      ...
      window.dispatchEvent(
        new CustomEvent<string>("yamet:ai-attach-file", { detail: path }),
      );
      openPanel();
      focusInput(null);
    },
    ...
    (request: AgentLaunchRequest) => {
      ...
      const hooksReady = launcher.supportsHooks
        ? invoke("agent_enable_hooks", {
            ...
            console.warn(
              `[yamet] could not enable ${request.agent} notifications:`,
              error,
            );
          })
        : Promise.resolve();
      ...
      for (const leafId of agentLeafIds) {
        void (async () => {
          ...
          if (!writeToSession(leafId, `${command.command}\r`)) {
            console.error(
              `[yamet] agent terminal ${leafId} closed before launch`,
            );
          }
        })();
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      unlisten = await listen<string[]>("yamet:open-file", (e) => {
        openAll(e.payload);
      });
            ...
            >
              ...
              >
                <div className="flex h-full min-h-0 flex-col border-r border-border/60 bg-card">
                  <div
                    key={sidebarView}
                    className="min-h-0 flex-1 yamet-panel-in"
                  >
