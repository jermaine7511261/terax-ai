...
export function ThemesSection() {
  ...
  const handleBgFiles = async (files: FileList | null) => {
    ...
    if (!file.type.startsWith("image/")) {
      setBgError(translate("settingsThemes.notImage", { name: file.name }));
      return;
    }
    try {
  ...
  return (
    <div className="flex flex-col gap-6">
      <SectionHeader
        title={translate("settingsThemes.themes")}
        description={translate("settingsThemes.description")}
      />
      ...
      >
        <div className="flex items-center justify-between">
          <Label>{translate("settingsThemes.themes")}</Label>
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              {translate("settingsThemes.create")}
            </Button>
            <Button
              variant="outline"
              size="sm"
            ...
            >
              {translate("settingsThemes.importTheme")}
            </Button>
          </div>
          <input
            ref={fileInputRef}
            type="file"
            accept=".yamet-theme,.json,application/json"
            className="hidden"
            onChange={(e) => {
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 flex-col">
            <Label>{translate("settingsAppearance.editorTheme")}</Label>
            <span className="text-[11px] text-muted-foreground">
              {translate("settingsThemes.editorThemeHint")}
            </span>
          </div>
          <Select
          ...
          >
            ...
            <SelectContent>
              <SelectItem value={EDITOR_THEME_AUTO} className="text-[12px]">
                {translate("settingsThemes.autoMatch")}
              </SelectItem>
              <SelectSeparator />
              {[...EDITOR_THEMES]
                .sort(
      ...
      >
        <div className="flex items-center justify-between">
          <Label>{translate("settingsAppearance.background")}</Label>
          <div className="flex items-center gap-2">
            {backgroundKind === "image" && backgroundImageId ? (
              ...
              >
                {translate("settingsThemes.remove")}
              </Button>
            ) : null}
            <Button
              variant="outline"
            ...
            >
              {backgroundKind === "image"
                ? translate("settingsThemes.replaceImage")
                : translate("settingsThemes.chooseImage")}
            </Button>
            <input
              ref={bgInputRef}
              type="file"
        ...
        {backgroundKind === "image" && backgroundImageId ? (
          <div className="flex flex-col gap-3 rounded-lg border border-border/60 p-3">
            <div className="flex items-center justify-between gap-3">
              <span className="text-[11.5px] text-muted-foreground">
                {translate("settingsThemes.opacity")}
              </span>
          ...
          <p className="text-[11px] text-muted-foreground">
            {translate("settingsThemes.dropHint")}
          </p>
        )}
      </div>
    </div>
