...
const LOCAL_META: Partial<Record<ProviderId, LocalMeta>> = {
  "llama.cpp": {
    ...
    description:
      "Local GGUF models via llama.cpp's HTTP server (llama-server --port 8080).",
    modelHint: (
      <>
        The model id loaded in llama.cpp — see the server's{" "}
...
export function ModelsSection() {
  ...
  return (
    <div className="flex flex-col gap-7">
      <SectionHeader
        title={t("settingsModels.models")}
        description={t("settingsModels.description")}
      />
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <Label>{t("settingsModels.providers")}</Label>
          <AddProviderMenu
        ...
        {visibleProviders.length === 0 && customEndpoints.length === 0 ? (
          <div className="rounded-lg border border-dashed border-border/60 bg-card/40 px-4 py-8 text-center">
            <p className="text-[12px] text-muted-foreground">
              {t("settingsModels.noProviders")}
            </p>
            <p className="mt-0.5 text-[10.5px] text-muted-foreground/70">
              {t("settingsModels.noProvidersHint")}
            </p>
          </div>
        ) : (
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="min-w-55 p-1">
        ...
        <DropdownMenuLabel className="px-2 text-[10px] tracking-wide text-muted-foreground uppercase">
          {t("settingsModels.localAndCustom")}
        </DropdownMenuLabel>
        {local.map((p) => (
          <ProviderMenuItem key={p.id} provider={p} onAdd={onAdd} />
        ))}
        ...
        >
          ...
          <span>{t("settingsModels.openaiCompatible")}</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
...
function AutocompleteRow({
  ...
  return (
    <>
      <FieldRow label={t("settingsModels.autocomplete")}>
      ...
      {enabled ? (
        <FieldRow label={t("settingsModels.trigger")}>
          <Select
            value={trigger}
            onValueChange={(v) =>
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="auto">
                {t("settingsModels.triggerAuto")}
              </SelectItem>
              <SelectItem value="manual">
                {t("settingsModels.triggerManual", {
                  shortcut: aiCompleteShortcut || t("settingsModels.shortcut"),
                })}
              </SelectItem>
            </SelectContent>
          </Select>
      ...
      {enabled && !hasKey ? (
        <p className="pl-19 text-[10.5px] text-muted-foreground">
          ...
          {t("settingsModels.notConnectedHint")}
        </p>
      ) : null}
    </>
  );
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-border/60 bg-card/60 px-3 py-2.5">
      ...
      <div className="mt-0.5 flex flex-col gap-2.5">
        {noBaseURL ? null : (
          <FieldRow label={t("settingsModels.baseUrl")}>
        ...
        <FieldRow label={t("settingsModels.modelId")}>
          <Input
            value={modelDraft}
        ...
        <FetchModelsRow
          ...
          onPick={(id) => void setModelId(id)}
        />

        {setContextLimit ? (
          <FieldRow label={t("settingsModels.context")}>
        ...
        {supportsKey ? (
          <FieldRow label={t("settingsModels.apiKey")}>
            {compatKey ? (
              <div className="flex flex-1 items-center gap-1.5">
                ...
                <Button
                  ...
                  title={t("settingsModels.removeKey")}
                  className="size-7 text-muted-foreground hover:text-destructive"
                >
                  <HugeiconsIcon
                    icon={Cancel01Icon}
              ...
              <div className="flex flex-1 gap-1.5">
                <Input
                  ...
                  placeholder={t("settingsModels.apiKeyOptional")}
                  spellCheck={false}
                  className="h-8 flex-1 font-mono text-[11.5px]"
                />
                <Button
    ...
    <div className="flex flex-col rounded-lg border border-border/60 bg-card/60">
      ...
      >
        ...
        <span className="text-[12.5px] font-medium truncate">
          {endpoint.name || t("settingsModels.openaiCompatible")}
        </span>
        ...
        <Button
          ...
          title={t("settingsModels.removeEndpoint")}
      ...
      {expanded && (
        <div className="flex flex-col gap-2.5 border-t border-border/40 px-3 py-2.5">
          <FieldRow label={t("settingsModels.name")}>
            <Input
              value={nameDraft}
              ...
              placeholder={t("settingsModels.myEndpoint")}
              spellCheck={false}
          ...
          <FetchModelsRow
            ...
            apiKey={endpointKey}
            onPick={(id) => void onUpdate({ modelId: id })}
          />
...
function FetchModelsRow({
  ...
  const { t } = useI18n();
  const { models, loading, error, refresh } = useFetchedModels(
    provider,
    baseURL,
    apiKey,
  );
  ...
  return (
    <div className="flex flex-col gap-1">
      <Button
        ...
        className="h-7 w-fit gap-1.5 px-2.5 text-[11px]"
      >
        <HugeiconsIcon icon={Refresh01Icon} size={12} strokeWidth={1.75} />
        {loading ? t("common.loading") : t("settingsModels.fetchModels")}
      </Button>
      {error ? (
        <span className="text-[10px] text-destructive">{error}</span>
      ) : null}
      {!loading && !error && models.length === 0 && baseURL.trim() ? (
        <span className="text-[10px] text-muted-foreground">
          {t("settingsModels.noFetchedModels")}
        </span>
      ) : null}
      {models.length > 0 ? (
        <div className="max-h-40 overflow-y-auto rounded-md border border-border/40 bg-muted/20">
          {models.map((m) => (
            <button
              key={m.id}
              type="button"
              onClick={() => onPick(m.id)}
              className="flex w-full items-center justify-between gap-2 px-2 py-1 text-left text-[11px] transition-colors hover:bg-accent"
            >
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-border/60 bg-card/60 px-3 py-2.5">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[12.5px] font-medium">{t("settingsModels.voiceInput")}</span>
      </div>

      <FieldRow label={t("settingsModels.provider")}>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
      ...
      <p className="text-[10.5px] leading-relaxed text-muted-foreground">
        {t("settingsModels.whispercppDesc")}
      </p>
