  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <Label>{t("settingsLsp.languageServers")}</Label>
...
function ServerRow({
  ...
  const status =
    detected === undefined
      ? t("settingsLsp.checking")
      : detected
        ? detected
        : t("settingsLsp.notFoundOnPath");
    ...
    >
      <div className="flex items-center gap-1.5">
        <button
          ...
          title={t("settingsLsp.detectAgain")}
        >
        ...
        {custom ? (
          <button
            ...
            title={t("settingsLsp.removeServer")}
          >
        ...
        <Switch
          ...
          aria-label={t("settingsLsp.toggleServer", {
            state: switchState.checked
              ? t("settingsLsp.disableWord")
              : t("settingsLsp.enableWord"),
            name: server.name,
          })}
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" className="h-6 px-2 text-[11px]">
          {t("settingsLsp.addCustomServer")}
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-sm">
            {t("settingsLsp.customTitle")}
          </DialogTitle>
        </DialogHeader>
        <div className="flex flex-col gap-2.5">
          {field(t("common.name"), name, setName, "Zig")}
          {field(t("settingsLsp.command"), command, setCommand, "zls")}
          {field(t("settingsLsp.args"), args, setArgs, "--stdio")}
          {field(
            t("settingsLsp.fileExtensions"),
            extensions,
            setExtensions,
            "zig, zon",
          )}
          {field(t("settingsLsp.languageId"), languageId, setLanguageId, "zig")}
          {field(
            t("settingsLsp.rootMarkers"),
            rootMarkers,
            setRootMarkers,
            "build.zig",
          )}
