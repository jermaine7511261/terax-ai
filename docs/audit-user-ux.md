...
| 原发现 | 严重度 | 修复状态 | 修复提交 |
|---|---|---|---|
| 1.1 首启只有一个 chat 标签，未自动打开终端 | P0 | ✅ 已修复 | `ed637d3`（首启注入冷终端标签并激活） |
| 4.2 Git 提交提示全英文混排 | P1 | ✅ 已修复 | `7f82882`（→ t() 双语键） |
| 6.1 硬编码英文 UI 大量残留、门禁只扫中文 | P0 | ✅ 已修复 | `7f82882`（16+ 处英文 → t()/tStatic） |
| Onboarding「设置→AI」与标签「模型」不符 | P1 | ✅ 已修复 | `80109b1`（术语统一 + README/排障同步） |
| 设置页「Connected」徽标未验证密钥即显示 | P1 | ✅ 已修复 | `3ad272d`（新增云端 key 测试连接） |

> 未发现的安全类 UX 项均已闭环。空状态组件沿用内嵌紧凑文本（整面板级空态已被 AiChat/PreviewPane 正确使用 Empty*）。
