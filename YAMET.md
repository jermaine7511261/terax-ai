...
- `net::web_fetch::*`（`web_fetch`）：**Grok grok-build 移植的网页抓取工具**——SSRF 双门控（DNS 每跳复检防 rebinding）、域名白名单（`modules/net/web_fetch/config.rs` 约 60 个开发文档域，host+path 前缀 O(1) 匹配，白名单外拒绝）、HTTPS 升级、HTML→markdown（htmd）、TTL 缓存、10MB 上限。`allow_local` 默认 false（fail-closed），仅显式 loopback 可被显式启用。对应 AI 工具面 `fetch_url`/`web_search`（免 key DuckDuckGo）/`deep_search`（Grok deep-research 4 阶段编排），三者只读不进写路径。
