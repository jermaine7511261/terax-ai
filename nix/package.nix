...
let
  ...
  # TODO: point at the fork's release host once binaries are published, e.g.
  # "github.com/your-org/yamet". Prebuilt artifacts are fetched from there.
  releaseHost = "github.com/your-org/yamet";

  srcMap = {
    x86_64-linux = fetchurl {
      url = "https://${releaseHost}/releases/download/v${version}/Yamet_${version}_amd64.deb";
      hash = sources.hashes.x86_64-linux;
    };
    x86_64-darwin = fetchurl {
      url = "https://${releaseHost}/releases/download/v${version}/Yamet_x64.app.tar.gz";
      hash = sources.hashes.x86_64-darwin;
    };
    aarch64-darwin = fetchurl {
      url = "https://${releaseHost}/releases/download/v${version}/Yamet_aarch64.app.tar.gz";
...
assert lib.assertMsg (builtins.hasAttr sys srcMap)
  "yamet: unsupported platform ${sys}";
...
stdenv.mkDerivation {
  ...
  installPhase = if stdenv.hostPlatform.isLinux then ''
    ...
    install -Dm755 usr/bin/yamet $out/bin/yamet

    wrapProgram $out/bin/yamet \
      "''${gappsWrapperArgs[@]}" \
