# Contributing to Yamet

Yamet is a solo-maintained project with a strong product direction. Contributions are welcome, but **alignment matters more than volume**.
...
- Yamet has one active maintainer.
...
For the architecture and how to contribute safely, see [YAMET.md](YAMET.md) and the [docs/ index](docs/README.md).

## Where to discuss

Use GitHub Issues for design discussion, scope questions, "should I work on X?", quick feedback, and tracking concrete bugs and features.
...
Yamet positions itself as **lightweight, fast, production-grade**. Every PR is reviewed against:
...
The most common way a PR breaks Yamet is a **local fix with global blast radius**: the diff solves one reported case, reads fine, passes type-check and clippy, and silently breaks the same subsystem in every other case. Review alone does not catch these. A test does.
...
- Yamet is not trying to be a full IDE replacement (VS Code, Cursor, Zed).
